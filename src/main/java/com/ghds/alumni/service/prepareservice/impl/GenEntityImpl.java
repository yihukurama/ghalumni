...
public class GenEntityImpl implements IGenEntity {
    ...
    public int genEntity(Table table) {
        ...
        if (tableName.startsWith("business_")) {
            entityFile = new File(PrepareConstant.businessEntityPath + File.separator + entity.getEntFileName());
        } else {
            entityFile = new File(PrepareConstant.adminEntityPath + File.separator + entity.getEntFileName());
        }
        ...
        if (entity.getEntColumns().contains("parentId")) {
            tplFile = new File(PrepareConstant.ENTITYTPL_PATH);
        }
    ...
    private Entity table2Entity(Table table) {
        ...
        if (table.getName().startsWith("business_")) {
            entity.setPackages(PrepareConstant.businessEntityPackage);
        } else {
            entity.setPackages(PrepareConstant.adminEntityPackage);
        }
        return entity;
    }
