...
import com.ghds.alumni.service.prepareservice.base.*;
...
public class GenMapper implements IGenMapper {
    ...
    public int genBaseMapper(Table table) {
        ...
        if (tableName.startsWith("business_")) {
            mapperFile = new File(PrepareConstant.businessMapperPath + File.separator + mapper.getMapperName());
        } else {
            mapperFile = new File(PrepareConstant.adminMapperPath + File.separator + mapper.getMapperName());
        }


        LogUtil.DebugLog(this.getClass(), "mapperFile is " + mapperFile.getPath());
        File tplFile = new File(PrepareConstant.MAPPERTPL_PATH);
        ...
        rfs.replacePackage(mapperFile.getAbsolutePath(), "${entityPackage}", mapper.getEntityPackage());
    ...
    private Mapper table2Mapper(Table table) {
        ...
        mapper.setMapperName(domainName + "Mapper.java");
        ...
        if (table.getName().startsWith("business_")) {
            mapper.setPackages(PrepareConstant.businessMapperPackage);
            mapper.setEntityPackage(PrepareConstant.businessEntityPackage);
        } else {
            mapper.setPackages(PrepareConstant.adminMapperPackage);
            mapper.setEntityPackage(PrepareConstant.adminEntityPackage);
        }
